...
const Usuarios = ({ formik, setId }) => {
   ...
   const handleNomina = async (name, value) => {
      if (value.length >= 6) {
         ...
         if (response.length > 0) {
            ...
            formik.current.setFieldValue("DenominacionCargo", puesto);
         } else {
   ...
   const Form = () => {
      const { roles, intengrantes,adscripcion } = Request({ peticiones: ["roles", "intengrantes",'adscripcion'] });
